...
test_ct_path = '/home/luosy/lsycode/LITS2017/LITS2017_test'   #需要预测的CT图像
seg_result_path = '/home/luosy/lsycode/LITS2017/test_liver' #需要预测的CT图像标签，如果要在线提交codelab，需要先得到预测过的70例肝脏标签

pred_path = '/home/luosy/lsycode/LITS2017/2dunet_tumor'
...
def main():
    val_args = parse_args()
# /home/luosy/lsycode/liver_seg_pipeline/models/LiTS_Unet_lsy/2021-10-29-16-25-56
    args = joblib.load('models/LiTS_Unet_lsy/2021-10-31-10-09-45/args.pkl')
    print('Config -----')
    for arg in vars(args):
        print('%s: %s' %(arg, getattr(args, arg)))
    print('------------')
    joblib.dump(args, 'models/LiTS_Unet_lsy/2021-10-31-10-09-45/args.pkl')

    # create model
    print("=> creating model %s" %args.arch)
    # model = Unet.U_Net(args)
    model = EUNet.EU1_Net(args)
    # model = network.deeplabv3plus_resnet50(num_classes=2,output_stride=16)
    model = torch.nn.DataParallel(model).cuda()

    model.load_state_dict(torch.load('models/LiTS_Unet_lsy/2021-10-31-10-09-45/0.9599246014232801_model.pth'))
    model.eval()
    print("load success!")
    #import sys
    #sys.exit()
    ...
    for file_index, file in enumerate(os.listdir(test_ct_path)):
        ...
        ct_crop = ct_array[start_slice:end_slice+1,:,:]
        ...
        with torch.no_grad():
            for n_slice in range(ct_crop.shape[0]-3):
                ...
                probability_map = np.zeros([1, 512, 512], dtype=np.uint8)
                #预测值拼接回去
                # i = 0
                ...
                slice_predictions[n_slice+start_slice+1,:,:] = probability_map
