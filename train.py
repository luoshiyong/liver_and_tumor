...
def train(args, train_loader, model, criterion, optimizer, epoch,criterion2):
    ...
    for i, (input, target,distarget) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if args.deepsupervision:
            ...
            loss2 = criterion2(edge_224[0],distarget[0])
    ...
    with torch.no_grad():
        for i, (input, target,distarget,filename) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            if args.deepsupervision:
                ...
                # print("output shape = ",output.shape)
                loss1 = criterion(output, target)
                loss2 = criterion2(edge_224, distarget)
                loss = loss2 + loss1
                iou = iou_score(output, target)
                dice_1 = dice_coef(output, target)
                ...
                im.convert('L').save('outputdir/seg/'+filename[0]+'.jpg')
                im1.convert('L').save('outputdir/dismap/' + filename[0] + '.jpg')
...
def main():
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', verbose=1, factor=0.1, patience=8,cooldown=20, min_lr=3e-7)
