...
import pandas as pd
from net import Unet,EUNet,res_unet_plus,R2Unet,sepnet
...
def parse_args():
    ...
    parser.add_argument('--load_model', default="C:/pycode/LITS2017-main1-master/models/epoch233-0.9736-0.8648_model.pth")
...
def main():
    ...
    train_img_paths = glob('/home/luosy/lsycode/LITS2017/data512_3/image/*')
    train_mask_paths = glob('/home/luosy/lsycode/LITS2017/data512_3/mask/*')
    val_img_paths = glob('/home/luosy/lsycode/LITS2017/data512_3/val_image/*')
    val_mask_paths = glob('/home/luosy/lsycode/LITS2017/data512_3/val_mask/*')
    ...
    model = Unet.EAU_Net(args)
    #model = EUNet.EU2_Net(args)
    # model = network.deeplabv3_resnet50(num_classes=2,output_stride=16)
    model = torch.nn.DataParallel(model).cuda()
    model.load_state_dict(torch.load("/home/luosy/lsycode/liver_seg_pipeline/models/LiTS_Unet_lsy/2021-11-04-09-56-47/0.9629999369869343_model.pth"))
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer,eta_min=0,T_max=18)
    ...
    for epoch in range(args.epochs):
        ...
        print('loss %.4f - iou %.4f - dice %.4f  - val_loss %.4f - val_iou %.4f - val_dice %.4f'
                  %(train_log['loss'], train_log['iou'], train_log['dice_1'], val_log['loss'], val_log['iou'], val_log['dice_1']))
        ...
        if val_loss < best_loss:
            torch.save(model.state_dict(), 'models/{}/{}/{}_model.pth'.format(args.name,timestamp,val_log['dice_1']))
            best_loss = val_loss
